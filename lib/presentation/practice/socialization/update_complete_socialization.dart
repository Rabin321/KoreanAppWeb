...
class UpdateCompleteSocialization extends StatefulWidget {
  String? id;
  PracticeSocializationModel? practiceSocializationModel;
  UpdateCompleteSocialization({
    Key? key,
    this.id,
    this.practiceSocializationModel,
  }) : super(key: key);

  @override
  State<UpdateCompleteSocialization> createState() =>
      _UpdateCompleteSocializationState();
}

class _UpdateCompleteSocializationState
    extends State<UpdateCompleteSocialization> {
  ...
  @override
  void initState() {
    title.text = widget.practiceSocializationModel!.title;
    correctAnswer.text = widget.practiceSocializationModel!.correctAnswer;
    pickedImage = XFile(widget.practiceSocializationModel!.imageUrl);
    for (int i = 0; i < 4; i++) {
      options[i] = widget.practiceSocializationModel!.options[i];
    }
                ...
                child: Column(
                  children: [
                    ...
                    EcoTextField(
                      labelText: "Title",
                      controller: title,
                      hintText: "This will used as tts",
                      validate: (v) {
                        if (v!.isEmpty) {
  ...
  void save() async {
    ...
    if (pickedImage == null) {
      ...
      if (parsedValue < 1 || parsedValue > 4) {
        ...
        await uploadImages();
        await PracticeSocializationModel.updatePracticeSocialization(
          widget.id!,
  ...
  Future postImages(XFile? imageFile) async {
    ...
    Reference ref = FirebaseStorage.instance
        .ref()
        .child("practice_socialization_Images")
